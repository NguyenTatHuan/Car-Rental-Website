...
import com.springprojects.service.elasticsearch.carType.CarTypeSearchService;
...
@RestController
@RequestMapping("/api/admin/car-type")
...
public class AdminCarTypeController {
    ...
    @GetMapping("/search")
    public ResponseEntity<List<CarTypeDocument>> searchCarTypes(@RequestParam("keyword") String keyword) {
        if (keyword == null || keyword.trim().isEmpty()) {
            return ResponseEntity.badRequest().build();
        }
        return ResponseEntity.ok(carTypeSearchService.searchByKeyword(keyword.trim()));
    }
